...
abstract class TweetSet extends TweetSetInterface:
  ...
  def descendingByRetweet: TweetList = 
    //println(s"mostRetweeted: $mostRetweeted")
    //println(s"without mostRetweeted: ${println(remove(mostRetweeted))}")
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet:
  ...
  def mostRetweeted: Tweet = 
    var highest: Tweet = elem
    foreach( f => if highest.retweets < f.retweets then highest = f )
    highest
  ...
  /**
   * filterAcc takes an accumulator as a second argument
   * 
   * 
  */
  def filterAcc(p: Tweet => Boolean, acc: TweetSet): TweetSet =
  ...
  // 
  // Tweet("user", "hola ...", 0).incl(t2)
  // Tweet("user2", "hola faiaojf...", 0)
  // NonEmpty( 1, Empty(), user2)
